...
export default function LoginModal({ open, onClose }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { accessToken, accSeq, accId, accName, accType, mainImagePath } = response.data; // 응답 데이터 구조에 맞게 구조분해할당

      if (accessToken) {
          localStorage.setItem("accessToken", accessToken);
          // 사용자 정보를 JSON 문자열로 변환하여 저장
          localStorage.setItem("userInfo", JSON.stringify({ accSeq, accId, accName, accType, mainImagePath }));
